...
estimate_cate <- function(Y, Z, X, learner = "ols", x_pop = NULL){
  ...
  # outputs: 
    # a n-by-2 (default) science table for the experiment or N-by-2 (if x_pop) is given science table for the population
  prop_score <- mean(Z) # the known, marginal treatment assignment probability or "propensity score," which we assume is uniform for all subjects
  model_data <- data.frame(Y=Y, Z=Z, X=X)
  stopifnot(
    "X must be an (n-by-p) matrix" = is.matrix(X),
    "x_pop must be an (N-by-p) matrix or nothing" = is.matrix(x_pop))
  if(is.null(x_pop)){
    x_pop = X # if the population covariates aren't provided, just estimate CATEs within the experiment
  }
  if(learner == "ols"){
    mod_ctl <- lm(Y ~ X, subset = (Z == 0))
    mod_trt <- lm(Y ~ X, subset = (Z == 1))
    E_y_0 <- predict(mod_ctl, newdata = data.frame(X = x_pop))
    E_y_1 <- predict(mod_trt, newdata = data.frame(X = x_pop))
    CATEs <- E_y_1 - E_y_0
  } else if(learner == "rf"){
    # causal forest fit using default parameters
    # see: https://grf-labs.github.io/grf/reference/causal_forest.html
    cf <- causal_forest(X=X, Y=Y, W=Z, W.hat=prop_score)
    CATEs <- predict(cf, newdata = data.frame(X = x_pop))
    }
  else {
    stop("learner must be either \"ols\" or \"rf\")")
  }
  CATEs
}
