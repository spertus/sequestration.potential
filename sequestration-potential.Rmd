...
nrcs_cates_ols <- data.frame(X = baseline_soc_grid, Y = cate_estimates_ols)
...
nrcs_cates_rf <- data.frame(X = baseline_soc_grid, Y = cate_estimates_rf)
...
nrcs_cate_plot <- ggplot(nrcs_data, aes(x = TC_2016, y = TC_2019-TC_2016, color = treatment_long)) +
  geom_point(size = 1.5) + 
  geom_line(data = nrcs_cates, aes(x = X, y = Y), color = "forestgreen", size = 1.2) +
